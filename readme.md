...
神经网络用于实现复杂的函数，非线性激活函数可以使神经网络随意逼近复杂函数。

神经网路的激活函数必须使用**非线性**。线性函数的问题，不管如何加深层数，总存在与之等效的“无隐藏层神经网路”。

例子：考虑将线性函数 $h(x)=cx$ 做为激活函数，把 $y(x) = h(h(h(x)))$ 的运算对应3层神经网络。这个运算会进行 $y(x)= c*c*c*x$ 的乘法运算，但同样可以理解为 $y(x) = ax$ （其中 $a = c^3$ ) 一次乘法运算，既没有隐藏层的神经网络。
...
模型：![image-20240222130917935](./assets/image-20240222130917935.png)
